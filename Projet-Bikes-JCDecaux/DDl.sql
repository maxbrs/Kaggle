...
CREATE PROCEDURE ADD_BIKE_STATION (IN v_name INT, IN v_lat NUMERIC(8), IN v_lon NUMERIC(8), IN v_address VARCHAR(100), IN v_available_bike_stands INT(4), IN v_available_bikes INT(4), IN v_banking BOOL, IN v_bonus BOOL, IN v_bike_stands INT(4), IN v_city VARCHAR(20), IN v_number INT(5), IN v_status enum('CLOSED','OPEN'), IN v_time DATETIME);
	BEGIN
		DECLARE v_sta_ID INT;
		DECLARE v_bik_ID INT;
		--DECLARE v_bik_sta_ID INT; = v_sta_ID
